...
3.  Go to your home folder and make a folder called `intro_scrna_dscov` (`cd ~` and `mkdir intro_scrna_dscov`), then type `pwd` to get the path to this folder.
...
It will ask for a File name -- paste this into the box: \`\`
...
If we look at the unique values in `orig.ident`, we see that there's only `pbmc3k` samples.
...
We recommend RPCA during integrative analysis where: A substantial fraction of cells in one dataset have no matching type in the other, datasets originate from the same platform (i.e. multiple lanes of 10x genomics), or there are a large number of datasets or cells to integrate.
...
The size of the dots indicates how many cells are in each cluster and the color indicates the resolution used. At the top is showing us the clustering results from `resolution = 0`, and moving down the tree we can see clusters splitting out into smaller sub-clusters as we reach `resolution = 1.2`. 
